...
void rbtree_foreach(struct rbtree* tree, foreach_func_t fn) {
    if (!tree || !tree->root || !fn)
        return;
    fn(tree->root->key, tree->root->data);
    struct rbtree_node * left_node = rbtree_next(tree->root);
    do{
        if(left_node)
            fn(left_node->key, left_node->data);
        else
            break;
        while (left_node = rbtree_next(left_node)) {
            fn(left_node->key, left_node->data);
        }
    }while(0);

    struct rbtree_node * right_node = rbtree_prev(tree->root);
    do{
        if(right_node)
            fn(right_node->key, right_node->data);
        else
            break;
        while (right_node = rbtree_prev(right_node)) {
            fn(right_node->key, right_node->data);
        }
    }while(0);
}
